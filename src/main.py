"""
AI Auto Annotator v5 — Windows Edition
...
class App(ctk.CTk):
    ...
    def update_dataset_class_counter(self):
        ...
        for anns in self.all_anns.values():
            for ann in anns:
                counts[ann.cls_name] += 1

        # Clear old rows
        for widget in self.counter_stats_frame.winfo_children():
            widget.destroy()

        if not counts:

            ctk.CTkLabel(
                self.counter_stats_frame,
    ...
    def __init__(self):
        super().__init__()
        self.title("AI Auto Annotator v5  |  Windows  |  YOLOv8 Offline")
        ...
        self.review_conf_v = tk.DoubleVar(value=0.50)  # default human review threshold (50%)
        self.review_list_data = []                     # holds tuples of (file_path_str, Annotation)
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkLabel(sb, text="v5 · Smart Train · Offline · Windows",
        ...
        self.model_v = ctk.StringVar(value=best_model)
        ...
        self.review_lbl = ctk.CTkLabel(cf, text="Review Threshold  50%",
                                       font=ctk.CTkFont(size=11), text_color=TEXT_PRI)
        self.review_lbl.pack(anchor="w", pady=(4, 0))
        ctk.CTkSlider(cf, from_=0.0, to=1.0, variable=self.review_conf_v,
                      progress_color=ACCENT2, button_color=ACCENT2,
                      command=lambda v: [self.review_lbl.configure(text=f"Review Threshold  {int(float(v)*100)}%"), self._on_review_conf_changed()]
                      ).pack(fill="x", pady=2)
        ...
        # ==============================
        # DATASET CLASS COUNTER PANEL
        # ==============================

        self.counter_frame = ctk.CTkFrame(
            sb,
            fg_color="#101722",
            corner_radius=8,
            border_width=1,
        ...
        ctk.CTkLabel(
            self.counter_frame,
            text="Target: Minimum 30 samples per class",
            font=ctk.CTkFont(size=9),
            text_color="#AAAAAA"
        ).pack(anchor="w", padx=8, pady=(0,4))

        # Dynamic stats area
        self.counter_stats_frame = ctk.CTkFrame(
            self.counter_frame,
            fg_color="transparent"
        )
        self.counter_stats_frame.pack(
            fill="x",
            padx=6,
            pady=(0,6)
        )
        ...
        self.epoch_v = tk.IntVar(value=100)
    ...
    def _build_center(self):
        ...
        af = ctk.CTkFrame(main,fg_color=BG_CARD,height=220,corner_radius=0)
        af.grid(row=5,column=0,sticky="ew"); af.grid_propagate(False)

        # Tabbed interface for Detections vs Human Review
        self.tabview = ctk.CTkTabview(af, segmented_button_fg_color=BG_DARK,
        ...
        # Tab 1: Current Detections (Original flow)
        tab_dets = self.tabview.add("Current Detections")
        tab_dets.grid_columnconfigure(0, weight=1)
        tab_dets.grid_rowconfigure(0, weight=1)

        self.det_lb = tk.Listbox(tab_dets, bg=BG_DARK, fg=TEXT_PRI,
                                 selectbackground="#1F3A5F", selectforeground=ACCENT,
                                 font=("Courier", 10), relief="flat",
                                 highlightthickness=0, borderwidth=0,
                                 activestyle="none", selectmode=tk.EXTENDED,
                                 exportselection=False)
        self.det_lb.grid(row=0, column=0, padx=4, pady=2, sticky="nsew")
        self.det_lb.bind("<<ListboxSelect>>", self._on_det_select)

        br = ctk.CTkFrame(tab_dets, fg_color="transparent")
        br.grid(row=1, column=0, padx=6, pady=4, sticky="e")
        ...
        # Tab 2: Human Review (Low confidence area)
        tab_review = self.tabview.add("Human Review")
        tab_review.grid_columnconfigure(0, weight=1)
        tab_review.grid_rowconfigure(0, weight=1)

        review_main_frame = ctk.CTkFrame(tab_review, fg_color="transparent")
        review_main_frame.grid(row=0, column=0, sticky="nsew")
        review_main_frame.grid_columnconfigure(0, weight=1)
        review_main_frame.grid_rowconfigure(0, weight=1)

        self.review_lb = tk.Listbox(review_main_frame, bg=BG_DARK, fg=TEXT_PRI,
                                    selectbackground="#1F3A5F", selectforeground=ACCENT,
                                    font=("Courier", 10), relief="flat",
                                    highlightthickness=0, borderwidth=0,
                                    activestyle="none", selectmode=tk.SINGLE,
                                    exportselection=False)
        self.review_lb.grid(row=0, column=0, padx=4, pady=2, sticky="nsew")
        self.review_lb.bind("<<ListboxSelect>>", self._on_review_item_select)

        rbr = ctk.CTkFrame(review_main_frame, fg_color="transparent")
        rbr.grid(row=0, column=1, padx=10, pady=4, sticky="ns")

        self.btn_accept = ctk.CTkButton(rbr, text="✔️ Accept", height=32, width=110,
                                        fg_color="#1A2A1A", hover_color="#223022", text_color=SUCCESS,
                                        font=ctk.CTkFont(size=11, weight="bold"), corner_radius=6,
                                        command=self._review_accept)
        self.btn_accept.pack(pady=4, fill="x")

        self.btn_reject = ctk.CTkButton(rbr, text="❌ Reject", height=32, width=110,
                                        fg_color="#2D1B1B", hover_color="#3D2020", text_color=DANGER,
                                        font=ctk.CTkFont(size=11, weight="bold"), corner_radius=6,
                                        command=self._review_reject)
        self.btn_reject.pack(pady=4, fill="x")

        self.btn_rename_rev = ctk.CTkButton(rbr, text="✏️ Rename", height=32, width=110,
                                            fg_color="#1A1A2E", hover_color="#252540", text_color=ACCENT3,
                                            font=ctk.CTkFont(size=11, weight="bold"), corner_radius=6,
                                            command=self._review_rename)
        self.btn_rename_rev.pack(pady=4, fill="x")
    ...
    def _save_persistent_config(self):
        try:
            ...
            config_data = {
                ...
                "review_conf_v": self.review_conf_v.get(),
    ...
    def _load_persistent_config(self):
        try:
            ...
            if config_path.exists():
                ...
                self.review_conf_v.set(config_data.get("review_conf_v", 0.50))
                if hasattr(self, "review_lbl"):
                    self.review_lbl.configure(text=f"Review Threshold  {int(self.review_conf_v.get()*100)}%")
    ...
    def train_model(self):
        annotated=[k for k,v in self.all_anns.items()
                   if any(str(p)==k for p in self.file_list)]
        if len(annotated)<2:
        ...
        def _run():
            try:
                ...
                random.seed(42)
                annotated_shuffled = list(annotated)
                random.shuffle(annotated_shuffled)
                split_idx = max(1, int(len(annotated_shuffled) * 0.8))
                if split_idx == len(annotated_shuffled) and len(annotated_shuffled) > 1:
                    split_idx -= 1
                train_keys = set(annotated_shuffled[:split_idx])
                ...
                for key in annotated:
                    ...
                    shutil.copy2(key,str(tmp/"images"/split/(stem+Path(key).suffix)))
                    lines=[]
                    ...
                    (tmp/"labels"/split/(stem+".txt")).write_text("\n".join(lines))
                    written+=1
                ...
                try:
                    self.after(0, lambda: self.status_v.set("Deriving optimal confidence..."))
                    m = YOLO.custom.val(data=str(tmp/"data.yaml"), verbose=False)
                    self._optimal_conf = 0.45
                except Exception:
                    self._optimal_conf = 0.45
    ...
    def _on_review_conf_changed(self):
        self._upd_stats()

    def _refresh_review_list(self):
        if getattr(self, "_updating_review", False):
            return
        self._updating_review = True
        try:
            selected_idx = None
            ...
            if sel:
                selected_idx = sel[0]
                if selected_idx < len(self.review_list_data):
                    selected_item = self.review_list_data[selected_idx]

            self.review_lb.delete(0, tk.END)
            self.review_list_data = []

            threshold = self.review_conf_v.get()

            for p in self.file_list:
                k = str(p)
                anns = self.all_anns.get(k, [])
                for a in anns:
                    if not a.manual and a.confidence < threshold:
                        self.review_list_data.append((k, a))

            self.review_list_data.sort(key=lambda x: x[1].confidence)

            for i, (k, a) in enumerate(self.review_list_data):
                fn = Path(k).name
                self.review_lb.insert(tk.END, f"  [{i:02d}] {fn:<15} | {a.cls_name:<12} ({int(a.confidence*100)}%)")

            if selected_item:
                try:
                    new_idx = self.review_list_data.index(selected_item)
                    self.review_lb.selection_set(new_idx)
                    self.review_lb.see(new_idx)
                except ValueError:
                    pass
        finally:
            self._updating_review = False

    def _on_review_item_select(self, event=None):
        if getattr(self, "_updating_review", False):
            return
        self._updating_review = True
        try:
            ...
            if not sel: return
            idx = sel[0]
            if idx >= len(self.review_list_data): return

            file_path_str, ann = self.review_list_data[idx]
            file_path = Path(file_path_str)
            if file_path in self.file_list:
                file_idx = self.file_list.index(file_path)
                if file_idx != self.file_index:
                    self.file_lb.selection_clear(0, tk.END)
                    self.file_lb.selection_set(file_idx)
                    self.file_lb.see(file_idx)
                    self._load_by_index(file_idx)
    ...
    def _review_accept(self):
        sel = self.review_lb.curselection()
        if not sel:
            messagebox.showinfo("Human Review", "Select a low-confidence annotation from the review list first.", parent=self)
            return
        idx = sel[0]
        file_path_str, ann = self.review_list_data[idx]
        ...
        self.has_unsaved_changes = True
        self.status_v.set(f"Accepted detection '{ann.cls_name}' as manual annotation.")
        ...
        next_idx = min(idx, len(self.review_list_data) - 1)
        if next_idx >= 0:
            self.review_lb.selection_clear(0, tk.END)
            self.review_lb.selection_set(next_idx)
            self.review_lb.see(next_idx)
            self._on_review_item_select()
    ...
    def _review_reject(self):
        ...
        if file_path_str in self.all_anns:
            self.all_anns[file_path_str] = [a for a in self.all_anns[file_path_str] if a != ann]

        self.has_unsaved_changes = True
        self.status_v.set(f"Rejected and removed detection '{ann.cls_name}'.")
    ...
    def _review_rename(self):
        ...
        if not self.user_cls:
            messagebox.showinfo("Rename Class", "Add classes first (＋ Add) in the manual annotation sidebar.", parent=self)
            return
        ...
        def choose_class(name):
            ann.cls_name = name
            ann.cls_id = self._cls_id(name)
            ann.manual = True
            ann.confidence = 1.0
            self.has_unsaved_changes = True
            self.status_v.set(f"Renamed and accepted detection as '{name}'.")
            self._upd_stats()
            self._redraw()
            self._color_file_list()
        ...
        for cn in self.user_cls:
            menu.add_command(label=cn, command=lambda name=cn: choose_class(name))
    ...
    def _upd_stats(self):
        ...
        self.update_dataset_class_counter()
